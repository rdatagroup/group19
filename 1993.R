...
#filtering out confederations for 1999
...
data_sorted <-ddply( CONMEBOL,.(country_full),
                    summarize,sum=sum(previous_points))

newdata5 <- data_sorted[order(-data_sorted$sum), ]
newdata5


#total perfomnce in Year 1999

data_sorted_1999 <-ddply(data2,.(country_full),
                    summarize,sum2=sum(previous_points))
data_sorted_1999
#sort in descending order
desc <- data_sorted_1999[order(-data_sorted_1999$sum2), ]
desc
